...
Comme la valeur de la statistique calculée est supérieure à celle critique à $5\%$ ($-2.089789 > -3.566$), on accepte $H_0$ et on conclut que le PGD est DS. 
...
Comme la valeur de la statistique calculée est supérieure à celle critique ($-2.825383 > -5.67$), on accepte $H_0$ et on conclut que le PGD est DS avec date de rupture dans la constante en $2005$ (position $37$ issue du test avec une date de rupture).
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
Box.test(ts_diff, lag = 9, type = "Ljung-Box")
```
...
On part d'un modèle ARMA(9,9) et on enlève, un à un, les coefficients non-significatifs jusqu'à tous soient significatifs. 
